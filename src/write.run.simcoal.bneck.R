...
simc.write<-function(basefile,num_rep,num_mark,migMat,pop_sizes,time_coalesc, time_b,size_bb){
	filename<-basefile
	...
	write("//Number of migration matrices : 0 implies no migration between demes\n2",file=filename,append=T)
	write("//Migration rates matrix  0",file=filename,append=T)
	write(t(migMat),ncolumns=length(pop_sizes),file=filename,append=T); migMat0<-migMat; migMat0[,]<-0
	write("//Migration rates matrix  1",file=filename,append=T)
	write(migMat0,ncolumns=length(pop_sizes),file=filename,append=T)
	...
	write(paste(length(pop_sizes)*2,"historical events"),file=filename,append=T)
	#calculates the population that all other populations coalesce back into
	for (p in 1:length(pop_sizes)) write(paste(time_b,p-1,p-1,"0",(size_bb/pop_sizes[p]),"0 0",sep=" "),file=filename,append=T)
	#calculates the population that all other populations coalesce back into
	for (p in 1:length(pop_sizes)) write(paste(time_coalesc,p-1,(tail(which(c(pop_sizes)>0),1)-1),"1 1 0 1",sep=" "),file=filename,append=T)
...
simc.run<-function(basefile,num_rep,m_core){
	filename<-basefile
	...
	if (.Platform$OS.type=="unix"){		
		if (m_core==T)
			err <- system(paste("./fsc -i ", filename, " -c28 -B28 -n ", num_rep, " -g -q",sep=""))
		if (m_core==F)
