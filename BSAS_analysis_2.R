...
setwd("C:/Users/seanmh/Dropbox/Projects/IN_PROGRESS/Best_sampling_across_systems/")
...
for (scen in 1:length(scenarios_ran)){	
	...
	#for (nrep in 1:num_reps){
		...
		for (trees_samp in n_trees){
			...
			#as long as its not missed (0) it counts as being caught (also try 5)
...
#This will count the NAs- important to report- NEED TO LOOK AT THIS RESULT AND REPORT
...
#PDFs showing each factor for the four kinds of alleles
	#Can do separately for the three kinds of bottlenecks by looking at rows for means_caught
#in the inner loop just paste in the 1:392 etc. above for rows of means_caught and sim_params
...
#can copy these rows into the below boxplot code to subset by bottleneck, otherwise will do across all bottlenecks
sim_paramsB1<-sim_params[1:392,]
sim_paramsB25<-sim_params[393:784,]
sim_paramsB5<-sim_params[785:1176,]
...
#So we need to control for or parcel out this factor, to see if they have some influence
...
#TO CHANGE CODE Change the 7 to a 5 to look at migration rate
#TO CHANGE CODE Change the 5:8 to 1:4 to look at catch 1 or catch 5
...
#BIG RESULT ITS ABOUT 3.5 across the board!!
